...
from ymago.models import GenerationJob, GenerationResult, BatchSummary
...
class TestCLIGCTIntegration:
    ...
    def sample_config(self):
        from ymago.config import Auth, Defaults, Settings, CloudTasksConfig
        return Settings(
    ...
    def test_image_generate_with_backend_option(self, sample_config):
        ...
        with patch("ymago.cli.load_config", new_callable=AsyncMock) as mock_load:
            with patch("ymago.cli.CloudTasksExecutionBackend") as mock_backend_cls:
    ...
    def test_image_generate_with_local_backend(self, sample_config):
        ...
        with patch("ymago.cli.load_config", new_callable=AsyncMock) as mock_load:
            with patch("ymago.cli.process_generation_job", new_callable=AsyncMock) as mock_process:
                mock_load.return_value = sample_config
        ...
        with patch("ymago.cli.load_config", new_callable=AsyncMock) as mock_load:
            with patch("ymago.cli.CloudTasksExecutionBackend") as mock_backend_cls:
                with patch("ymago.cli.parse_batch_input", side_effect=mock_requests):
                    ...
                    result = self.runner.invoke(
                        app, ["batch", "run", str(input_file), "-o", str(tmp_path), "--backend", "cloud-tasks"]
                    )
    ...
    def test_batch_run_with_local_backend(self, sample_config, tmp_path):
        ...
        mock_summary = BatchSummary(
            total_requests=1, successful=1, failed=0, skipped=0,
            processing_time_seconds=1.0, results_log_path="log.jsonl",
            throughput_requests_per_minute=60.0
        )
        
        with patch("ymago.cli.load_config", new_callable=AsyncMock) as mock_load:
            with patch("ymago.cli.LocalExecutionBackend") as mock_backend_cls:
                with patch("ymago.cli.parse_batch_input", side_effect=mock_requests):
                    mock_load.return_value = sample_config
                    mock_backend = MagicMock()
                    mock_backend.process_batch = AsyncMock(return_value=mock_summary)
