...
class TestCloudTasksExecutionBackendSkeleton:
    ...
    async def test_backend_initialization(self):
        ...
        config = Settings(
            ...
            cloud_tasks=CloudTasksConfig(
                gct_project_id="test-project", worker_url="https://worker.example.com"
            ),
        )
    ...
    async def test_process_batch_returns_summary_as_stub(self, tmp_path):
        ...
        summary = await backend.process_batch(
            mock_requests(), output_dir=tmp_path, concurrency=5, rate_limit=60
        )
    ...
    async def test_submit_calls_create_task_per_job(self):
        ...
        with patch.object(
            backend, "_create_gct_task", new_callable=AsyncMock
        ) as mock_create:
            ...
            assert all(
                r.get_metadata("execution_backend") == "cloud-tasks" for r in results
            )
            assert all(
                r.local_path == Path("cloud-task-dispatched").resolve() for r in results
            )
